...
class ImagePrefetchService extends GetxService {
  ...
  void _processQueue() {
    while (_prefetchQueue.isNotEmpty &&
        _activePrefetches < _maxConcurrentPrefetch) {
      final nextUrl = _prefetchQueue.removeAt(0);
      if (!_prefetchedUrls.contains(nextUrl) &&
          !_prefetchingUrls.contains(nextUrl)) {
        unawaited(_executePrefetch(nextUrl));
      }
    }
  }

  /// Prefetch images for a list of properties
  Future<void> prefetchPropertyImages(
    ...
    properties.take(limit).forEach(collect);

    void queue(String url) => unawaited(prefetchImage(url));
    urls.forEach(queue);
  ...
  Future<void> prefetchPropertyDetailImages(Property property) async {
    ...
    if (images != null) {
      void collect(PropertyImage image) {
...
mixin ImagePrefetchMixin {
  ...
  void prefetchDetailImages(Property property) {
    if (Get.isRegistered<ImagePrefetchService>()) {
      unawaited(imagePrefetchService.prefetchPropertyDetailImages(property));
    }
  }
}
