...
@GenerateMocks([AuthRepository, TokenService, SessionController])
...
void main() {
  ...
  group('AuthController initialization', () {
    ...
    setUp(() {
      ...
      when(mockSessionController.ready).thenAnswer((_) async {});
      when(mockSessionController.isAuthenticated).thenReturn(false.obs);
      when(mockSessionController.rememberMe).thenReturn(false.obs);
  ...
  group('FormValidationController', () {
    ...
    test('should validate email format correctly', () {
      final result =
          validationController.validateEmailOrPhone('test@example.com');
      expect(result, isNull);
    });

    test('should reject invalid email format', () {
      final result =
          validationController.validateEmailOrPhone('invalid-email');
    ...
    test('should validate password length', () {
      ...
      final validPassword =
          validationController.validatePassword('password123');
      expect(validPassword, isNull);
    });

    test('should validate confirm password matches', () {
      final mismatch =
          validationController.validateConfirmPassword('pass1', 'pass2');
      expect(mismatch, isNotNull);

      final match = validationController.validateConfirmPassword(
        'password',
        'password',
      );
      expect(match, isNull);
